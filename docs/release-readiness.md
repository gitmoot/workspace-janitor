...
- [ ] Record source commit, Go toolchain/dependency versions and build commands. CI tests GitHub's temporary PR merge ref, then checks out and verifies the **full submitted head** separately for the artifact; otherwise the artifact would embed an ephemeral merge SHA and would not reproduce from the reviewed PR head. `make release-linux-amd64` uses `CGO_ENABLED=0 GOOS=linux GOARCH=amd64`, read-only modules, trimmed paths, disabled VCS stamp and build ID, and commit-derived version/commit metadata (no wall-clock build date). Compare two builds and verify `dist/SHA256SUMS` against the binary.
